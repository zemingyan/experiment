...
public class Louvain implements Cloneable {
    ...
    FUEdge[] new_FU_edge;   //新的邻接表
    int[] new_head;
    int new_top = 0;
    final int iteration_time = 3; // 最大迭代次数

    FUEdge global_FU_edge[];   //全局初始的临接表  只保存一次，永久不变，不参与后期运算
    int global_head[];
    int global_top = 0;

    void addEdge(int u, int v, double weight) {
        if (FUEdge[top] == null)
            FUEdge[top] = new FUEdge();
        FUEdge[top].v = v;
        FUEdge[top].weight = weight;
        FUEdge[top].next = head[u];
        head[u] = top++;

    }

    void addNewEdge(int u, int v, double weight) {
        if (new_FU_edge[new_top] == null)
            new_FU_edge[new_top] = new FUEdge();
        new_FU_edge[new_top].v = v;
        new_FU_edge[new_top].weight = weight;
        new_FU_edge[new_top].next = new_head[u];
        new_head[u] = new_top++;
    }

    void addGlobalEdge(int u, int v, double weight) {
        if (global_FU_edge[global_top] == null)
            global_FU_edge[global_top] = new FUEdge();
        global_FU_edge[global_top].v = v;
        global_FU_edge[global_top].weight = weight;
        global_FU_edge[global_top].next = global_head[u];
    ...
    boolean try_move_i(int i) { // 尝试将i加入某个簇
        ...
        for (int j = head[i]; j != -1; j = FUEdge[j].next) {
            int l = cluster[FUEdge[j].v]; // l是nodeid所在簇的编号
            edgeWeightPerCluster[l] += FUEdge[j].weight;
        }
        ...
        for (int j = head[i]; j != -1; j = FUEdge[j].next) {
            int l = cluster[FUEdge[j].v]; // l代表領接点的簇号
            if (vis[l]) // 一个領接簇只判断一次
                continue;
            vis[l] = true;
    ...
    void rebuildGraph() { // 重构图
        ...
        for (int u = 0; u < new_n; u++) { // 将同一个簇的挨在一起分析。可以将visindex数组降到一维
            ...
            for (int i = 0; i < nodeInCluster[u].size(); i++) {
                ...
                for (int k = head[t]; k != -1; k = FUEdge[k].next) {
                    int j = FUEdge[k].v;
                    ...
                    if (u != v) {
                        ...
                        delta_w[v] += FUEdge[k].weight;
                    }
                }
                new_node_weight[u] += node_weight[t];
            }
            for (int k = new_head[u]; k != -1; k = new_FU_edge[k].next) {
                int v = new_FU_edge[k].v;
                new_FU_edge[k].weight = delta_w[v];
            }
        }
